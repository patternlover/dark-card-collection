...
| R3 | **Replatform Medusa — F3**: target **Oracle Cloud Free Tier** (Docker compose api+worker+redis+Caddy, guida `DEPLOYMENT.md`); code-prep **done** (Dockerfile, `docker-compose.prod.yml`, Caddyfile, backup pg_dump, env prod, feed `/api/feed/products`, **email Resend subscriber**); **deploy+cutover = open** (VM Oracle + DNS + Neon nuovo DB + Stripe webhook + enable stripe region + Vercel env + remove Payload + feed in Merchant) — richiede infra utente | code-prep done (2026-08-29: tsc backend ✓ · guida deploy pronta); deploy/cutover open |
