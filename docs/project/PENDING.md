...
| R2 | **Replatform Medusa — F2**: storefront su Medusa — **step 1 done** (`src/lib/medusa/*`, catalogo shop/PDP/espansioni/home re-punted, cart→Medusa, checkout Payment Element + success, analytics, subscriber `order.placed` snapshot); step 2: **account cliente** (login/register/My Account), contenuti statici (`src/config.ts`) | step 1 done (2026-08-29: tsc ✓ · test 104/104 ✓ · smoke ✓); step 2 open |
| R3 | **Replatform Medusa — F3**: deploy Railway (backend+worker) + Neon + Upstash, cutover env, **rimozione Payload** (commerce + CMS + dashboard OAuth), feed Google Merchant, sitemap/JSON-LD | open |
