...
| R3 | **Replatform Medusa — F3**: code-prep **done** (Dockerfile, railway.json, env prod, feed `/api/feed/products`, doc passi); **deploy+cutover = open** (Railway api+worker, Neon DB separato, Upstash, webhook Stripe, enable stripe su region, subscriber email Resend, remove Payload, feed in Merchant) — richiede infra utente | code-prep done (2026-08-29: tsc ✓ · feed XML ✓); deploy/cutover open |
