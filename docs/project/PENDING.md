...
| W4 (Drive) | **Scontrini lotti**: env complete in `.env.local` e Vercel (email client_email + JSON chiave + `GOOGLE_DRIVE_FOLDER_ID`). Auth verificata (JWT OK, cartella leggibile). **Bloccante: API Google Drive non abilitata** sul progetto (`dark-card-collection-505621` / id 187737126407) → abilitare da `https://console.developers.google.com/apis/api/drive.googleapis.com/overview?project=187737126407` e attendere la propagazione (pochi minuti) | blocked (enable API Drive — utente) |
