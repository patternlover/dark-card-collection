...
| R1 | **Replatform Medusa — F1**: modulo custom `procurement` (lotti, FIFO `effective_unit_cost`, costo medio, margini, sales channel esterni) + Admin routes/widgets + porting test `purchase-math`/`record-sale`/`inventory` (branch `feat/medusa-replatform`) | open |
| R2 | **Replatform Medusa — F2**: storefront su Medusa (`src/lib/medusa/*`), cart→Medusa, checkout Stripe, success page, analytics, **account cliente** (login/register/My Account), contenuti statici (`src/config.ts`) | open |
