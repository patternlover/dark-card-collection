...
| W4 (Drive) | **Scontrini lotti**: creare service account Google + cartella Drive condivisa e inserire nelle env (Vercel + locale) `GOOGLE_DRIVE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_DRIVE_SERVICE_ACCOUNT_PRIVATE_KEY` (con `\n` escaped), `GOOGLE_DRIVE_FOLDER_ID` — vedi `.env.example`. Senza env l'upload scontrino mostra errore nel modale | waiting-user (creazione credenziali utente) |
