...
| W4 (Drive) | **Scontrini lotti**: env `GOOGLE_DRIVE_SERVICE_ACCOUNT_EMAIL` (client_email esatto: `scontrini-dark-card-collection@dark-card-collection-505621.iam.gserviceaccount.com`) e `GOOGLE_DRIVE_SERVICE_ACCOUNT_PRIVATE_KEY` (JSON completo) già in `.env.local` e Vercel. **Manca `GOOGLE_DRIVE_FOLDER_ID`** (ID della cartella "Scontrini" condivisa col service account — da URL Drive). Con folder id l'upload drag&drop funziona | waiting-user (folder id) |
