...
| R3 | **Replatform Medusa — F3**: **CUTOVER COMPLETATO** ✅ (2026-09-05: merge su main, Payload rimosso — src/payload, migrations, /admin, /dashboard, api/stripe, deps — build+tsc+test ok, sito live su Medusa: /shop, /checkout, /account, feed, sitemap 200). ⚠️ **Checkout Stripe PAUSATO** (il Payment Element richiede ancora fix — ordine provato via API/system provider, manca il flusso browser). Resta: fix checkout Stripe · enable stripe region verifica · webhook Stripe · feed in Merchant | cutover done (2026-09-05: prod live Medusa 200 · Payload rimosso · tsc+test+build ok); checkout Stripe open |
