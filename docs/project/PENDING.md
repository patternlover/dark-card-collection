...
| M1 | **Rivisitazione modali dashboard** (branch `feat/dashboard-modals-redesign`, guida utente step by step, dettagli in `docs/project/sessions/2026-08-12-dashboard-modals-redesign.md`): step 1 Vendita Esterna **done** (select prodotto raggruppato per `title` + varianti in optgroup, `src/lib/sale-options.ts`); rimanenti: Magazzino → Nuovo/Duplica Prodotto · Listino → Modifica Prodotto · Lotti → Registra/Modifica Lotto · Categorie/Collezioni | in-progress |
