...
- **Checkout embedded**: il checkout usa Stripe Embedded Checkout lato client, quindi serve anche `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` (la chiave pubblicabile è la stessa di `STRIPE_PUBLISHABLE_KEY`; su Vercel va impostata con il prefisso `NEXT_PUBLIC_` per essere iniettata nel client).
