/**
 * Email conferma ordine — porting da `src/lib/order-email.ts` (Payload) al backend
 * Medusa. Invio via Resend HTTP API (nessuna dipendenza aggiuntiva).
 */
...
export async function sendOrderConfirmationEmail(data: OrderEmailData): Promise<void> {
  ...
  if (!apiKey) {
    console.log(`[order-email] RESEND_API_KEY non configurata — email a ${data.customerEmail} saltata`)
    return
  }
  ...
  if (!res.ok) {
    ...
    console.error(`[order-email] Resend ${res.status}: ${body?.message ?? "errore"}`)
  }
}